...
const Navbar = ({ cartCount }) => {
  ...
  console.log("Navbar cartCount:", cartCount); // Debugging purpose
      ...
      <div className="max-w-7xl mx-auto flex items-center justify-between h-16">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <div className="relative w-9 h-9 flex-shrink-0">
            <button className="flex items-center justify-center w-full h-full rounded-full hover:bg-gray-100 transition-colors">
              <ShoppingCart className="w-5 h-5 text-black" />
            </button>{""}
            {cartCount > 0 && (
              <span
                key={cartCount}
                className="absolute -top-1 -right-1 bg-gray-200 text-black text-[10px] font-bold rounded-full w-4 h-4 flex items-center justify-center"
              >
                {cartCount}
              </span>
            )}
          ...
          <a
            href="#"
            className="hidden md:block font-semibold text-black hover:text-primary transition-colors px-2"
          >
            Login
          </a>
